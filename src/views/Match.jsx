...
import { For, Index, Show, Switch, Match as MatchCase } from 'solid-js/web';
		...
		setEffects(state => {
			...
			for (let idx = 0; idx < effects.length; idx += 3) {
				...
				switch (kind) {
					...
					case 'Poison':
						newstate.effects ??= new Set(state.effects);
						newstate.effects.add(Text(state, newstate, id, `Poison ${param}`));
						break;
					case 'Delay':
						newstate.effects ??= new Set(state.effects);
					...
					case 'Freeze':
						newstate.effects ??= new Set(state.effects);
						newstate.effects.add(Text(state, newstate, id, `Freeze ${param}`));
						break;
					case 'Dmg':
						...
						handOverlay = () =>
							pl().casts === 0
								...
								: (pl().getStatus('nova') >= 2 ||
								  ...
								  (pl().id !== p1id() ||
										pl().handIds.some(id => {
											const card = g.Cards.Codes[g.get(id, 'card')];
											return card && card.isOf(g.Cards.Names.Nova);
										}))
								? 1
								: null;
